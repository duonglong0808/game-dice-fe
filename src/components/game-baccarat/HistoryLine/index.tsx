...
export function HistoryLineBaccarat({ baccaratId }: { baccaratId: number }): JSX.Element {
            ...
            <tr key={rowIndex} className={cx('table__tr')}>
              {Array.from({ length: 9 }, (v, k) => k + 1).map((colIndex) => (
                <td
                  ...
                  <div className="flex flex-col justify-between mx-auto flex-wrap w-[16.5px] h-[18px] p-[1px]">
                    {dataPosition[`${colIndex}${rowIndex}`]?.value?.map((item: any) => (
                      <span
                        className={cx('w-[2px] h-[7.5px] rotate-[45deg] rounded-lg', {
                          'bg-[#db1002]': item == 'red',
                          'bg-[#0403cb]': item == 'blue',
                        })}></span>
                    ))}
